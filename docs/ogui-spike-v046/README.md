...
- In `--overlay-mode`, the plugin mounts the same Armada control tree through OGUI's native `OverlayContainer`. Normal OGUI startup remains Quick Bar-first, so this path is an explicit overlay fallback rather than a second UI implementation.
