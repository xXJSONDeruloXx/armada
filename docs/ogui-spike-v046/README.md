...
- The Power and Fans cards include profile reset, underclock presets, fan curve and point editing, fan-stop, responsiveness, and protected curve management. The Games card includes Default/current-game targeting, inherited per-game FEX/thunk/performance settings, CPU lists, scheduler, priorities, and environment variables through native OGUI controls.
- The Compatibility card keeps Steam-private operations behind `armada-steam-call`: global and per-game Proton tools, global and per-game resolution, launch options, manual AppID loading, single-game reset, all-game reset, and the existing sweep path.
