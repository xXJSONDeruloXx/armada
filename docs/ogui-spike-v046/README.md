...
- Overlay mode applies Armada's existing `centeredChord` or `sideChord` preference to OGUI's InputPlumber activation target and reapplies it after device initialization. OGUI's native input handling remains the single gesture owner.
...
- The System card also edits the shared overlay layout, centered and slide-out activation chords, and edge-swipe settings through the shared atomic `overlay.json`.
...
The Armada `Containerfile` also exposes an explicit `armada-ogui` image target. It bundles the same runtime under `/usr/share/armada/ogui`, provides `/usr/bin/armada-opengamepadui` and enables the OGUI user service. The launcher seeds the Armada archive into OGUI's user plugin directory before starting; the default `armada` target remains unchanged and does not start OGUI.
