import QtQuick

QtObject {
    readonly property color panel: "#1b1d21"
    readonly property color panelRaised: "#262a30"
    readonly property color row: "#20242a"
    readonly property color rowFocused: "#315c86"
    readonly property color text: "#f0f0f0"
    readonly property color muted: "#aeb6c2"
    readonly property color accent: "#66c0f4"
    readonly property color error: "#d15d5d"
    readonly property color errorPanel: "#3b2225"
    readonly property color scrim: "#66000000"
    readonly property color transparent: "transparent"
    readonly property int spacing: 12
    readonly property int rowHeight: 64
    readonly property int navWidth: 52
    readonly property int iconSize: 26
    readonly property int radius: 3
    readonly property int borderWidth: 1
    readonly property int pageTitleSize: 30
    readonly property int bodySize: 20
    readonly property int replaceEnterMs: 100
    readonly property int replaceExitMs: 60
    readonly property int panelAnimationMs: 160
}
